...
* Principal: 앙클레이브를 실행하는 EC2 인스턴스에 부여된 IAM 역할(Role)의 ARN을 넣습니다.
* Action:
  * kms:Decrypt: 앞서 설명한 직렬 호출이나 봉투 암호화의 열쇠 복호화에 필요합니다.
  * kms:GenerateDataKey: 데이터를 암호화할 때 쓸 새 열쇠를 만들 때 필요합니다.
* Condition (가장 중요):
  * kms:RecipientAttestation:ImageSha384: 이 필드가 바로 앙클레이브의 지문(PCR0)을 대조하는 곳입니다.
  * 실습 시 nitro-cli build-enclave 명령 후 나오는 PCR0 해시값을 여기에 복사해서 넣으면 됩니다.
* PCR0 불일치: 코드를 한 줄이라도 수정해서 다시 빌드하면 PCR0 값이 바뀝니다. 코드를 고칠 때마다 KMS 정책의 해시값도 업데이트해야 한다는 점을 개발자들에게 강조하세요.

----
Attestation (신원 증명):
* 앙클레이브의 고유 지문인 PCR0 값이 어떻게 생성되는지 확인했습니다.
* KMS가 이 지문을 확인하여 "믿을 수 있는 코드"인지 판단하는 원리를 다뤘습니다.

Decryption (복호화) & 봉투 암호화:
* 비용 절감을 위해 데이터 키(Data Key)만 KMS에서 복호화해오는 봉투 암호화 로직을 Java 코드로 확인했습니다.
* KMS 호출 시 인풋(암호문+지문)과 아웃풋(평문 열쇠)의 흐름을 정리했습니다.

Security (보안성 시연):
* 부모 서버의 Root 관리자조차 앙클레이브 내부 메모리를 덤프하거나 프로세스를 볼 수 없다는 격리 시나리오를 구성했습니다.
